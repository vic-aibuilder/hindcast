...
_SUB_SLICE_CONTEXT: dict[str, str] = {
    "sneaker_streetwear": (
        ...
        "SNEAKER / STREETWEAR — CATEGORY BASELINE. "
        "This sub-slice (Nike, Adidas, New Balance, Kith, Aime Leon Dore, Flight Club, and similar) "
        "has built its visual identity on bold graphic branding: large logos, heavy signage, and overt "
        "brand expression are the historical norm — these brands are recognized in part by their logo "
        "presence. When evaluating saturation for this sub-slice, treat the following as historically "
        "expected: heavy or prominent signage; logo-forward / logo-dominant treatments; bold typography "
        "and graphic brand expression; industrial material references (concrete, steel, raw surfaces). "
        "Saturation findings become more interesting when they appear in places that contradict this "
        "graphic-forward baseline — minimalism, logo-absence, muted signage, or restraint where the "
        "category's identity would predict bold branding. A pattern noting the absence of something is "
        "most useful when that thing is something this category's brands are historically known for; "
        "surface that contrast explicitly (for example, a category built on logos now suppressing them) "
        "rather than simply noting frequency."
    ),
    "contemporary_fashion": (
        ...
        "CONTEMPORARY FASHION / QUIET LUXURY — CATEGORY BASELINE. "
        "This sub-slice (The Row, Toteme, Khaite, Acne Studios, and similar) has built its identity "
        "on restraint: minimal branding, neutral palettes, gallery-style layouts, and the deliberate "
        "suppression of overt signage. These are not departures for this category — they are the "
        "established norm. When evaluating saturation for this sub-slice, treat the following as "
        "baseline expectations, not findings: logo-absent or minimal signage; neutral / tonal color "
        "palettes; open-plan, gallery-density layouts; sculptural objects as the primary spatial gesture. "
        "Saturation findings become more interesting when they appear in places that contradict this "
        "restrained baseline — warmth, ornament, color, density, or overt brand expression where the "
        "category's convention would predict minimalism. A pattern noting the absence of something is "
        "most useful when that thing is something this category's brands would conventionally not do; "
        "surface that contrast explicitly rather than restating expected minimalism."
    ),
}
